...
import com.wingedtech.common.keycloak.client.TokenRelayRequestInterceptor;
import com.wingedtech.common.keycloak.security.AuthorizationHeaderUtil;
import feign.Client;
...
public class FeignClientConfigurer {
    ...
    public FeignClientConfigurer(Decoder decoder, Encoder encoder, Client client, Contract contract, AuthorizationHeaderUtil authorizationHeaderUtil) {
        this(decoder, encoder, client, contract, null, new ErrorDecoder.Default(), authorizationHeaderUtil);
    }

    public FeignClientConfigurer(Decoder decoder, Encoder encoder, Client client, Contract contract, LoadBalancedResourceDetails loadBalancedResourceDetails, AuthorizationHeaderUtil authorizationHeaderUtil) {
        this(decoder, encoder, client, contract, loadBalancedResourceDetails, new ErrorDecoder.Default(), authorizationHeaderUtil);
    }

    public FeignClientConfigurer(Decoder decoder, Encoder encoder, Client client, Contract contract, LoadBalancedResourceDetails loadBalancedResourceDetails, ErrorDecoder errorDecoder, AuthorizationHeaderUtil authorizationHeaderUtil) {
    ...
    private RequestInterceptor getInternalRequestInterceptor() {
        throw new UnsupportedOperationException("Keycloak体系下暂不支持internal client");
...
//        return internalRequestInterceptor;
    ...
    public RequestInterceptor getUserFeignClientInterceptor() {
        return new TokenRelayRequestInterceptor(this.authorizationHeaderUtil);
    }
    ...
    protected Feign.Builder getPublicFeignClientBuilder() {
        return getBasicBuilder().requestInterceptor(getPublicFeignClientInterceptor());
    }

    public RequestInterceptor getPublicFeignClientInterceptor() {
        return new PublicFeignClientInterceptor();
    }

    public <T> T buildPublicFeignClientWithDecode404(Class<T> clazz, String serviceName) {
        return buildOnTarget(clazz, serviceName, getPublicFeignClientBuilder().decode404());
    }
}
