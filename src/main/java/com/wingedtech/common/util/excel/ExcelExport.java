...
import com.wingedtech.common.util.excel.annotation.ExcelField.Align;
...
public class ExcelExport implements Closeable {
    ...
    public ExcelExport(String title, Class<?> cls, Type type, String... groups) {
        this(null, null, title, cls, type, false, groups);
    }
    ...
    public ExcelExport(String sheetName, String title, Class<?> cls, Type type, String... groups) {
        this(null, sheetName, title, cls, type, false, groups);
    }
     ...
     * @param groups    导入分组
     */
    public ExcelExport(String sheetName, String title, Class<?> cls, Type type, boolean createEmptyRow, String... groups) {
        this(null, sheetName, title, cls, type, createEmptyRow, groups);
    }
    ...
    public ExcelExport(Workbook wb, String sheetName, String title, Class<?> cls, Type type, String... groups) {
        this(wb, sheetName, title, cls, type, false, groups);
    }
     ...
     * @param cls       实体对象，通过annotation.ExportField获取标题
     * @param type      导出类型（1:导出数据；2：导出模板）
     * @param createEmptyRow
     * @param groups    导入分组
     */
    public ExcelExport(Workbook wb, String sheetName, String title, Class<?> cls, Type type, boolean createEmptyRow, String... groups) {
        if (wb != null) {
            this.wb = wb;
        } else {
            this.wb = createWorkbook();
        }
        this.createSheet(sheetName, title, cls, type, createEmptyRow, groups);
    }
    ...
    public ExcelExport(Workbook wb, String sheetName, String title, List<String> headerList, List<Integer> headerWidthList) {
        ...
        this.createSheet(sheetName, title, headerList, headerWidthList, false);
    }
     ...
     * @param groups              导入分组
     */
    public void createSheet(String sheetName, String title, Class<?> cls, Type type, boolean createEmptyRow, String... groups) {
        ...
        // 创建工作表
        this.createSheet(sheetName, title, headerList, headerWidthList, createEmptyRow);
    }
...
//		if (ef != null && (ef.type()==0 || ef.type()==type)){
     ...
     * @param createEmptyRow
     */
    public void createSheet(String sheetName, String title, List<String> headerList, List<Integer> headerWidthList, boolean createEmptyRow) {
...
//			}
    ...
    public <E> ExcelExport setDataRow(E e) {
        int colunm = 0;
        Row row = this.addRow();
     ...
     * @param fileName 输出文件名
     */
    public ExcelExport write(HttpServletResponse response, HttpServletRequest request, String fileName) throws UnsupportedEncodingException {
        response.reset();
        response.setContentType("application/octet-stream; charset=utf-8");
        response.setHeader(HttpHeaders.ACCESS_CONTROL_EXPOSE_HEADERS, HttpHeaders.CONTENT_DISPOSITION);
        response.setHeader(HttpHeaders.CONTENT_DISPOSITION, HeaderValuesUtils.contentDispositionValues(request, fileName));
        try {
            write(response.getOutputStream());
        } catch (IOException ex) {
            log.error(ex.getMessage(), ex);
