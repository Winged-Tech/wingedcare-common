 ...
 */
package com.wingedtech.common.reflect;
...
//			throw new IllegalArgumentException("在 [" + obj.getClass() + "] 中，没有找到 [" + methodName + "] 方法 ");
    ...
    public static Object transform(Object val, Class<?> c) {
        if (c == String.class){
            val = ObjectUtils.toString(val);
            if(StringUtils.endsWith((String)val, ".0")){
                val = StringUtils.substringBefore((String)val, ".0");
            }
        }else if (c == Integer.class){
            val = ObjectUtils.toInteger(val);
